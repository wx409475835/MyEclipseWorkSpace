	...
	-->
  	...
  	<script type="text/javascript">
		function var_Confirm(){
			alert("借阅成功！\n祝您阅读愉快！");
			window.location.href="${basePath }SelectBooks.do";
		}
	</script>
	<style type="text/css">
		table{
			  				...
			  				<tr>
			  					...
			  					<c:choose>
			  						<c:when test="${bk.book_count >=1 }">
				  						<center>
				  							<button type="button" style="width: 80px;height: 30px;border-radius:5px;color:#6B8E23;" onclick="var_Confirm()"><a href="${basePath }BrrowBooksAction.do?book_id=${bk.book_id }"><font color="#8B7355">借阅</font></a></button>
				  							<c:if test="${sessionScope.ident=='a' }">
				  								<button type="button" style="width: 80px;height: 30px;border-radius:5px;color:#9400D3;"><a href="${basePath }AlterBookInformationAction.do?book_id=${bk.book_id }"><font color="#A020F0">修改</font></a></button>
				  							</c:if>
				  						</center>
			  						</c:when>
			  						<c:otherwise>
			  							<center>
				  							...
				  							<c:if test="${sessionScope.ident=='a' }">
				  								<button type="button" style="width: 80px;height: 30px;border-radius:5px;color:#9400D3;"><a href="${basePath }AlterBookInformation?book_id=${bk.book_id }"><font color="#A020F0">修改</font></a></button>
				  							</c:if>
				  						</center>
			  						</c:otherwise>
			  						</c:choose>
