...
<html>
  ...
  <body>
  	<div class="showallcount-div">
  		ID&nbsp;&nbsp;<input type="text" name="haoto_id" placeholder="请输入需要查询的ID或名称">
  		...
  		<div class="table-top">
	   	...
	   	<div class="table-bottom">
	   		<div class="showallcount-div1">
		   		<table border="1px" cellpadding="0" cellspacing="0">
			   		...
			   		<tbody>
			   			<c:forEach items="${SelectAllHaoto_hAoto.list }" var="hAoto" varStatus="status">
	   		...
	   		function gotopage(pagenum,totalrecord){
	   			if(!isNaN(pagenum)){
	   				if(pagenum <= totalrecord){
	   					window.location.href='${basePath}SelectAllHaotoAction.do?currentpage='+pagenum;
	   				}else{
	   					alert('请输入合法页码');
	   				}
	   			}else{
	   	...
	   	<div class="table-bottom-bottom">
			<ul class="table-bottom-ul">
				<li class="table-ul-li"><a href="javascript:void(0)" onclick="gotopage(${SelectAllHaoto_hAoto.previouspage},${SelectAllHaoto_hAoto.totalpage})">上一页</a></li>
				<c:forEach var="pagenum" items="${SelectAllHaoto_hAoto.pagebar }">
					<c:if test="${pagenum == SelectAllHaoto_hAoto.currentpage }">
						<li class="table-ul-center-li-currentpage"><a href="javascript:void(0)">${pagenum }</a></li>
					</c:if>
					<c:if test="${pagenum != SelectAllHaoto_hAoto.currentpage }">
						<li class="table-ul-center-li"><a href="javascript:void(0)" onclick="gotopage(${pagenum },${SelectAllHaoto_hAoto.totalpage})">${pagenum }</a></li>
					</c:if>
				</c:forEach>
				<li class="table-ul-li"><a href="javascript:void(0)" onclick="gotopage(${SelectAllHaoto_hAoto.nextpage},${SelectAllHaoto_hAoto.totalpage })">下一页</a></li>
				<li class="table-ul-li-inp"><input type="text" id="page" maxlength="2" style="width: 30;border-radius: -webkit-calc(20px);" /></li>
				<li class="table-ul-li-btn"><button onclick="gotopage(document.getElementById('page').value,${SelectAllHaoto_hAoto.totalpage})">Go</button></li>
			</ul>
		</div>
  	</div>
  </body>
