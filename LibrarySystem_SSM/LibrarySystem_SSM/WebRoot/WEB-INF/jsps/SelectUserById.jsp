...
<html>
  <head>
    ...
    <title>My JSP 'SelectUserById.jsp' starting page</title>
  ...
  <body>
    <c:if test="${ sessionScope.Login!=null}">
    	<form method="post" action="SelectUserByIdAction.do">
    		<br/><br/>
    		<div class="div">
		  		<input placeholder="请输入需要的用户ID" name="ID" style="width: 180px;height: 35px;" type="text"/>
		  		<input type="submit" value="查询" style="width: 60px;height: 30px;color:#38B738;"/>
		  		<input class="aaa" type="submit" value="添加" formaction="Register.do"/> 
  			</div>
    	
  			<c:if test="${ sessionScope.SelectUserById!='[]' && sessionScope.SelectUserById !=null }">
  			...
  			<c:forEach items="${ sessionScope.SelectUserById }" var="pu">
  				<tr>
  					...
  					<td>
  						<button><a href="http://localhost:8888/LibrarySystem_SSM/UpdateUserAction.do?id=${pu.person_id }">修改用户</a></button>
  						<button><a href="http://localhost:8888/LibrarySystem_SSM/DeletePerson_UserAction.do?id=${pu.person_id }">删除用户</a></button>
  					</td>
  				</tr>
  			</c:forEach>
  		</table>
