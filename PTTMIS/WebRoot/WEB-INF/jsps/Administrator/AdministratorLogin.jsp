...
<html>
  <head>
		...
		$(document).ready(function(){	
			...
			$("[name='ha_authority']").change(function(){
				...
				if(index ==1){
					...
					$("[value='登陆']").css("width","200px").css("margin-top","20px").css("margin-left","0px");
					$("[value='登陆']").css("border-radius","6px").css("height","33px");
					$("form").prop("action","AdministratorLoginAction.do");
					bool=1;
				}else{
					$("[name='account']").parent().parent().css("display","none");
					$("[type='button']").css("display","block");
					$("[value='登陆']").css("width","").css("margin-top","0px");
					$("[value='登陆']").css("border-radius","6px").css("height","33pxs").css("margin-left","20px");
					$("form").prop("action","");
					bool=0;
				}
			});
	...
	<style type="text/css">
		...
		.login-div{
			...
			border-radius: -webkit-calc(20px);
			filter:alpha(opacity=80);  
		    -moz-opacity:0.8;  
		    -khtml-opacity: 0.8;  
		    opacity: 0.8; 
  ...
  <body background="images/Login.jpg">
    <form method="post" action="doctor/DoctorLoginAction.do">
    	<div class="login-div">
    		<div class="login-top-div">南阳市中心病人治疗跟踪管理信息系统</div>
    		<table>
	    		<tr id="tr1">
	    			<td>
